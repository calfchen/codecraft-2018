...
string DevCloud::OutPut(){
    ...
    //     return "";
    // }
    // if(m_predict_vm[0].vm_name == "flavor1" && m_predict_vm[0].vm_num > 500){
    //     return "";
    // }
    // if(StringToDatetime(m_predict_begin_time) - m_max_day == 8){
    //     return "";
...
PredictVM DevCloud::PredictOneKindVM_GuiZe(string flavor, int total_week){
    ...
    // vector<int> train = FindTrain(vm, total_week);
    ...
    for(int i = (7 - last_day); i < 7; i++){
        // if(week_factor[total_week-1][i] < 0.001){
        //     last_week_total += week_original_value[total_week-1][i];
        // }
        // else{
        //     last_week_total += week_original_value[total_week-1][i] / week_factor[total_week-1][i];
        // }
        last_week_total += week_original_value[total_week-1][i];
    }
...
void DevCloud::GuiZe(){
    ...
    for(unsigned int i = 0; i < m_vm_kind.size(); i++){
        ...
        if(p_vm.vm_num > 0){
            //对预测结果放大1.2倍
            // p_vm.vm_num = (int)(p_vm.vm_num * 2.0 + 0.5);
...
PredictVM DevCloud::PredictOneKindVM_ES(string flavor){
    ...
    ****************************************/
    ...
    for(int i = 0; i < m_predict_day; i++){
        double mean = accumulate(sum_day.begin(), sum_day.end(), 0) / (sum_day.size() * 1.0);
        int day = (int)(0.8 * mean + 0.5 * sum_day[sum_day.size() -1] + 0.5);
...
void DevCloud::Mean(){
    ...
    PredictVM p_vm;
    for(unsigned int i = 0; i < m_vm_kind.size(); i++){
        p_vm = PredictOneKindVM_Mean(m_vm_kind[i].vm_name, 10);
        if(p_vm.vm_num > 0){
            //对预测结果放大1.2倍
            // p_vm.vm_num = (int)(p_vm.vm_num * 1.2 + 0.5);
...
PredictVM DevCloud::PredictOneKindVM_Mean(string flavor, int day_num){
    //统计最后day_num的总量
    int begin = m_max_day - day_num + 1;
    map<int,int> vm = StatisticsOneKindVM(flavor);
    ...
    PredictVM p_vm;
    p_vm.vm_name = flavor;
    p_vm.vm_num = (int)(total_num / (day_num * 1.0) * m_predict_day + 0.5);
    return p_vm;
}
...
PredictVM DevCloud::PredictOneKindVM_GuiZe(string flavor){
    ...
    for(unsigned int i = 0; i < predict_day.size(); i++){
        ...
        // predict_day[i] = (int)(base * (factor_mean[i % 7] + factor_median[i % 7])/2.0+ 0.5);
...
void DevCloud::BIU(){
    string na[15] = {"flavor1","flavor2","flavor3","flavor4","flavor5","flavor6","flavor7","flavor8", \
                     "flavor9","flavor10","flavor11","flavor12","flavor13","flavor14","flavor15"};

    vector<string> name(na,na+15);

    // int b[15] = {8,3,2,3,9,5,2,7,9,5,4,9,3,4,0};
    // int b[15] = {23,17,24,26,24,9,20,3,12,9,4,1,22,13,6};
    int b[15] = {8,49,44,11,29,2,10,17,48,42,42,12,27,12,20};
    // int b[15] = {8,41,15,63,26,6,12,12,97,76,20,99,92,30,3};
    // int b[15] = {199,130,95,177,59,62,158,119,108,4,171,185,149,9,87};
    // int b[15] = {44,0,121,0,22,0,187,0,17,0,41,0,67,0,193};

    vector<int> num(b,b+15);
